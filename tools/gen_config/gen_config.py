...
parser.add_argument(
    "--concurrency_level",
    type=int,
    help="The number of threads for each party",
    default=1
)
...
bloom_filter_size = get_bf_size(
    args.set_size,
    2**-(args.false_positive_rate),
    args.number_of_parties,
    args.intersection_threshold)

number_of_hash_functions = get_number_of_hash_functions(2**-args.false_positive_rate)
...
if not args.no_print:
    ...
    print(f"The false positive rate is: 2^-{args.false_positive_rate}")
...
party_list = []
for i in range(0, args.number_of_parties):
    party_list += ["P" + str(i)]
...
config = {
    ...
    "falsePositiveRate": args.false_positive_rate,
    ...
    "concurrencyLevel": args.concurrency_level,
...
diffSeeds = []
for i in range(0, args.number_of_parties):
    config["sameNum"] = max([args.set_size - (i) * diff_step, same_amount])
...
bScript = "#! /bin/bash \n"
for i in range(1, args.number_of_parties):
    bScript += "./bin/main ./config/P" + \
               str(i) + "_config.json & \n"
bScript += "./bin/main ./config/P0_config.json"
...
for i in range(1, args.number_of_parties):
    bScript += "./bin/benchmark ./config/P" + \
               str(i) + "_config.json & \n"
bScript += "./bin/benchmark ./config/P0_config.json"
